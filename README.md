...
3.  **In-Window Fullscreen Button:**
    - **What it does:** Adds a new fullscreen button to the video action bar (next to the widescreen toggle) that stretches the in-page video embed to cover the viewport. This is not the browser-native fullscreen API; instead it applies fixed positioning and sizing to the embed so you get a distraction-free, full-viewport player.
    - **How to exit:** Press `Esc` to revert back to the normal player sizing.
    - **Where:** The button appears on any video watch page in the same action row as the other action buttons.
...
- **`widescreen-mode.js`:**
  ...
  - The button's click event toggles the preference, saves it to `localStorage`, and re-applies the style. The injected button now uses inline SVG icons (black monitor-style artwork) instead of text glyphs.

- **`fullscreen-mode.js`:**
  - Listens for the `ds-page-updated` event.
  - When on a watch page, it polls for the action bar and injects an In-Window Fullscreen button next to the widescreen toggle.
  - Clicking the button applies fixed, full-viewport inline styles to the video embed element (`.ds-video-section__embed`) to simulate fullscreen inside the page.
  - Pressing `Esc` exits the in-window fullscreen mode and reverts inline styles.
